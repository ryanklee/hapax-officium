...
from demo.models import get_config_dir, get_voice_examples_path, get_voice_profile_path
...
def load_style_guide(path: Path | None = None) -> dict:
    ...
    p = path or (get_config_dir() / "presenter-style.yaml")
    if not p.exists():
...
def load_voice_examples(path: Path | None = None) -> dict:
    ...
    p = path or get_voice_examples_path()
...
def load_voice_profile(path: Path | None = None) -> dict:
    ...
    p = path or get_voice_profile_path()
    if p is None or not p.exists():
