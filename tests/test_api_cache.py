...
class TestDataCache:
    ...
    async def test_refresh_populates_management(self):
        ...
        with (
            patch("logos.data.management.collect_management_state", mock_management),
            patch("logos.data.goals.collect_goals", MagicMock(return_value=None)),
            patch("logos.data.team_health.collect_team_health", MagicMock(return_value=None)),
            patch("logos.data.agents.get_agent_registry", MagicMock(return_value=[])),
            patch("logos.data.nudges.collect_nudges", MagicMock(return_value=[])),
        ):
            await c.refresh()
    ...
    async def test_refresh_populates_nudges(self):
        ...
        with (
            patch("logos.data.management.collect_management_state", MagicMock(return_value=None)),
            patch("logos.data.goals.collect_goals", MagicMock(return_value=None)),
            patch("logos.data.team_health.collect_team_health", MagicMock(return_value=None)),
            patch("logos.data.agents.get_agent_registry", MagicMock(return_value=[])),
            patch("logos.data.nudges.collect_nudges", MagicMock(return_value=[mock_nudge])),
        ):
            await c.refresh()
    ...
    async def test_refresh_tolerates_collector_errors(self):
        ...
        c = DataCache()
        with (
            patch("logos.data.management.collect_management_state", side_effect=Exception("boom")),
            patch("logos.data.goals.collect_goals", MagicMock(return_value=None)),
            patch("logos.data.team_health.collect_team_health", MagicMock(return_value=None)),
            patch("logos.data.agents.get_agent_registry", MagicMock(return_value=[])),
            patch("logos.data.nudges.collect_nudges", MagicMock(return_value=[])),
